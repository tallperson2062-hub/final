...
def _pbf_hdome_response(signed_f32, depth):
    ...
    recon = _pbf_reconstruction(seed, signed_f32, method='dilation')
...
PBF_STEP_DEFAULTS = {
    'step_5_illum_denoise': {
        'enabled': True,
        'bg_sigma': 30.0, 'bilat_d': 3, 'bilat_sc': 15.0, 'bilat_ss': 5.0,
    },
    'step_6_dog': {
        'enabled': True,
        'fine_sigma1': 0.5, 'fine_sigma2': 1.2,
        'coarse_sigma1': 1.0, 'coarse_sigma2': 5.0,
        'recon_radius': 1,
    },
    'step_7_sensitivity': {
        'enabled': True,
        'noise_k_fine': 2.7, 'noise_k_coarse': 1.0,
        'min_component_area_fine': 13, 'min_component_area_coarse': 470,
        'local_max_min_dist': 11, 'compactness': 0.0,
    },
    'step_8_shapefilter': {
        'enabled': True,
        'min_area': 1, 'max_area': 20000,
        'min_circularity': 0.6, 'min_solidity': 0.85,
        'max_aspect_ratio': 2.0, 'min_convexity': 0.00,
    },
    'step_9_extent': {
        'enabled': True,
        'close_radius_fine': 3, 'close_radius_coarse': 21, 'close_radius_extent': 3,
        'extent_k': 0, 'min_component_area_extent': 0, 'local_max_min_dist_extent': 1,
    },
    'step_10_rescue': {
        'enabled': True,
        'micro_rescue_ring_px': 2, 'micro_rescue_min_z': 3.9, 'micro_rescue_max_area': 195,
    },
}
...
def run_pbf_defect_pipeline(gray_u8, params):
    ...
    # ── ADDED: Fill white holes inside PBF red segments (Step 10 display) ───────
    # Detect all red pixels already painted on the final_overlay
    _fo_red_mask = (
        (final_overlay[:, :, 2] > 200) &   # high red channel
        (final_overlay[:, :, 1] < 30)  &   # low green channel
        (final_overlay[:, :, 0] < 30)       # low blue channel
    ).astype(np.uint8)
    if _fo_red_mask.max() > 0:
        # Label each connected red blob separately
        _fo_n, _fo_lmap = cv2.connectedComponents(_fo_red_mask, connectivity=8)
        for _fo_i in range(1, _fo_n):
            # Fill any interior holes in this red blob
            _fo_region = (_fo_lmap == _fo_i).astype(np.uint8)
            _fo_filled = ndi.binary_fill_holes(_fo_region).astype(np.uint8)
            # Repaint the filled region as solid red (BGR: 0, 0, 255)
            final_overlay[_fo_filled > 0] = (0, 0, 255)
    # ── END ADDED ────────────────────────────────────────────────────────────────
...
class ImageAnalyzerApp:
    ...
    def _batch3d_start(self):
        ...
        output_base = os.path.abspath(
            os.path.expanduser(str(self._tab3_output_var.get()).strip())
        )
        if not output_base or output_base == os.path.abspath('Not selected'):
        ...
        # Take immutable batch snapshots before starting worker threads.
        # Workers must not read or mutate live GUI/configuration state.
        batch_roi_points = (
            np.array(self.tab3_roi_points, dtype=np.int32, copy=True)
            if self.tab3_roi_points is not None else None
        )
        batch_config_data = copy.deepcopy(self.tab3_config_data)             if self.tab3_config_data is not None else {}

        # Tkinter must only be accessed from the main thread. Worker threads
        # place UI callbacks in this queue; the main thread drains it.
        ...
        def _pump_ui_queue():
            while True:
                ...
                try:
                    ...
                    print(f"[BATCH UI ERROR] {type(ui_exc).__name__}: {ui_exc}", flush=True)
            if self.tab3_processing or not ui_queue.empty():
                self.root.after(50, _pump_ui_queue)

        # Schedule the pump from the GUI/main thread before starting workers.
        _post_ui( _pump_ui_queue)
        ...
        def _run():
            ...
            self.root.after(
                0,
                lambda: self.tab3_log(
                    f"Batch worker started: {workers} worker(s), {total} image(s)"
                )
            )
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for done_idx, fut in enumerate(_as_completed(futures), start=1):
                    ...
                    try:
                        ...
                        print(
                            f"[BATCH WORKER ERROR] {fname}: "
                            f"{type(exc).__name__}: {exc}",
                            flush=True
                        )
                        msg = f"{fname} -> ERROR: {type(exc).__name__}: {exc}"
                        _post_ui( lambda m=msg: self.tab3_log(m))
                    del fut
                    gc.collect()
...
"""
    ...
    def init_tab2(self):
        ...
        # ── PERFORMANCE FIX: debounce state for live parameter controls ──
        # Scale/Spinbox widgets fire their `command` callback continuously
        # (on every pixel of drag / every keystroke), not just once the
        # user settles on a value. Without debouncing, each of those events
        # triggered a full 18-step pipeline recompute (process_all_steps)
        # AND a full parameter-panel widget rebuild, which is why manually
        # dragging a slider made the GUI crawl even though the same values
        # hardcoded in code (i.e. process_all_steps called once) were fast.
        # See _schedule_pipeline_update() / _run_scheduled_pipeline_update().
        self._pipeline_update_after_id = None
        self._PIPELINE_UPDATE_DEBOUNCE_MS = 200
    ...
    def _schedule_pipeline_update(self):
        """PERFORMANCE FIX: coalesce bursts of parameter-change events.

        ttk.Scale (and to a lesser extent Spinbox) call their `command`
        callback repeatedly while the user is interacting with them -- a
        single slider drag can fire dozens of events. Each of those used to
        trigger an immediate, synchronous, full 18-step pipeline recompute
        (process_all_steps, including the expensive Steps 5-10 PBF stages:
        multi-scale DoG, opening-by-reconstruction, watershed, etc.) plus a
        full parameter-panel widget rebuild -- on the single Tk GUI thread.
        That is what made the GUI "very slow" specifically when parameters
        were changed manually through the widgets, even though the exact
        same numeric values hardcoded in code (which only cause
        process_all_steps to run once) were fast.

        This method replaces the "recompute on every event" behaviour with
        a standard UI debounce: any pending scheduled recompute is
        cancelled and a new one is scheduled _PIPELINE_UPDATE_DEBOUNCE_MS
        in the future. As long as events keep arriving faster than the
        debounce window, nothing actually recomputes; only once the user
        pauses (e.g. releases the slider or stops typing) does the pipeline
        run -- exactly once. The algorithm, parameter semantics, and final
        displayed results are unchanged; only how often/when the (already
        correct) recompute happens is different.
        """
        if self._pipeline_update_after_id is not None:
            try:
                self.root.after_cancel(self._pipeline_update_after_id)
            except Exception:
                pass
        self._pipeline_update_after_id = self.root.after(
            self._PIPELINE_UPDATE_DEBOUNCE_MS, self._run_scheduled_pipeline_update)

    def _run_scheduled_pipeline_update(self):
        """Actually run the (debounced) pipeline recompute + redraw.

        Rebuilding the parameter-control panel here is skipped
        (rebuild_controls=False): the step hasn't changed, only a
        parameter value has, so the Scale/Spinbox the user is actively
        holding stays exactly as-is instead of being destroyed and
        recreated out from under the drag on every tick.
        """
        self._pipeline_update_after_id = None
        self.process_all_steps()
        self.display_current_step(rebuild_controls=False)
    ...
    def process_all_steps(self):
        ...
        self.step_data['masked_bgr'] = masked_image.copy()
        ...
        self.step_data['rotated_bgr'] = rotated_bgr.copy()
        ...
        self.step_data['markers'] = markers.copy()
        ...
        # ── ADDED: Step 17 – solid red segments on original grayscale (no white holes) ──
        # Use the labeled connected-component map from Step 16 to paint every detected
        # segment as 100 % solid red on top of the original (rotated) grayscale image.
        # binary_fill_holes removes any interior holes per segment before painting.
        _step17_labeled = self.step_data.get('labeled')
        if (
            _step17_labeled is not None and
            isinstance(_step17_labeled, np.ndarray) and
            int(_step17_labeled.max()) > 0
        ):
            # Grayscale base: same geometry as the pipeline input (post-rotate/flip)
            _gray17 = cv2.cvtColor(self.step_data['rotated_bgr'], cv2.COLOR_BGR2GRAY)
            result = cv2.cvtColor(_gray17, cv2.COLOR_GRAY2BGR)
            for _lbl17 in range(1, int(_step17_labeled.max()) + 1):
                # Binary mask for this one connected component
                _seg17 = (_step17_labeled == _lbl17).astype(np.uint8)
                # Fill any internal holes so the painted region is completely solid
                _seg17_filled = ndi.binary_fill_holes(_seg17).astype(np.uint8)
                # Paint solid red (BGR: 0, 0, 255) over the grayscale base
                result[_seg17_filled > 0] = (0, 0, 255)
        # ── END ADDED ────────────────────────────────────────────────────────────────
    ...
    def display_current_step(self, rebuild_controls=True):
        """Display the current step's image and parameters.

        rebuild_controls: when False, skip destroying/recreating the
        parameter-control panel (used by the debounced live-parameter
        update path, where the step hasn't changed -- only rebuilding the
        step image/labels is needed; see _run_scheduled_pipeline_update).
        """
        ...
        # Update parameter controls (skipped during debounced live-parameter
        # refreshes -- see rebuild_controls docstring above)
        if rebuild_controls:
    ...
    # NOTE (performance fix): every handler below used to call
    # `self.process_all_steps(); self.display_current_step()` directly. Since
    # these are wired to Scale/Spinbox `command=` callbacks that fire
    # continuously while the user interacts with the control, that meant one
    # full, expensive 18-step pipeline recompute (+ full parameter-panel
    # rebuild) per event -- dozens of them during a single slider drag. They
    # now call `self._schedule_pipeline_update()`, which debounces those
    # bursts into a single recompute after the user pauses. The parameter
    # value itself (self.params[...]) is still updated immediately/exactly
    # as before, so live numeric feedback in the Spinbox/Scale is unaffected
    # -- only the (already-correct) image recompute is deferred slightly.
    ...
    def update_step2_rotate(self, rot_var):
        ...
        try:
            ...
            self._schedule_pipeline_update()
        except ValueError:
    ...
    def tab3_start_batch_processing(self):
        ...
        # Snapshot configuration before starting worker threads.
        roi_points = (
            np.array(self.tab3_roi_points, dtype=np.int32, copy=True)
            if self.tab3_roi_points is not None else None
        )
        config_data = (
            copy.deepcopy(self.tab3_config_data)
            if self.tab3_config_data is not None else {}
        )
        ...
        def _run():
            ...
            import multiprocessing as _mp
            # Use a conservative worker count because each image allocates
            # several full-size NumPy/OpenCV arrays.
            workers = 1
            self.root.after(
                0,
                lambda: self.tab3_log(
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for done_idx, fut in enumerate(as_completed(futures), start=1):
                    ...
                    try:
                        ...
                        msg = f"{fname} -> ERROR: {type(exc).__name__}: {exc}"
                        self.root.after(0, lambda m=msg: self.tab3_log(m))
    ...
    def tab3_process_single_image(
        self,
        image_path,
        defects_folder,
        non_defects_folder,
        filename,
        roi_points_override=None,
        config_data_override=None
    ):
        """Process one image and save it to its classification folder.

        Optional override arguments are used by batch processing so worker
        threads never read or mutate live GUI configuration state.
        """
        import time as _time
        _started_at = _time.perf_counter()
        print(f"[BATCH START] {os.path.basename(str(image_path))}", flush=True)
        ...
        # Batch calls provide snapshots; direct single-image calls retain
        # the original GUI-state fallback behavior.
        _batch_snapshot_mode = config_data_override is not None
        _config_source = (
            config_data_override if _batch_snapshot_mode
            else self.tab3_config_data
        )

        if _batch_snapshot_mode:
            _roi_source = roi_points_override
        else:
            _roi_source = self.tab3_roi_points

        if _roi_source is not None:
            roi_points = np.array(_roi_source, dtype=np.int32, copy=True)
        elif _config_source and 'polygon_points' in _config_source:
            roi_points = np.array(
                _config_source['polygon_points'], dtype=np.int32
            )
        else:
        ...
        # Get a private parameter copy. Backfilling defaults must not mutate
        # the shared configuration dictionary across worker threads.
        if _config_source and 'pipeline_params' in _config_source:
            params = copy.deepcopy(_config_source['pipeline_params'])
        else:
            params = {
                'step_2_rotate_flip': {'enabled': True, 'rotate': 0, 'flip_h': False, 'flip_v': False},
                'step_3_grayscale': {'enabled': True},
        ...
        # ── ADDED: Eliminate white/bright holes inside drawn red segments ────────────
        # After all per-contour fills, some inter-component gaps and original-image
        # bright pixels may still show through inside what should be solid red regions.
        # Detect every red pixel already drawn, group into connected blobs, fill each
        # blob's interior holes completely, then repaint the filled area solid red.
        _b17_red_ch  = result[:, :, 2]   # Red channel (BGR layout)
        _b17_grn_ch  = result[:, :, 1]
        _b17_blu_ch  = result[:, :, 0]
        _b17_red_px  = (
            (_b17_red_ch > 200) &         # strong red
            (_b17_grn_ch < 30)  &         # no green
            (_b17_blu_ch < 30)             # no blue
        ).astype(np.uint8)
        if _b17_red_px.max() > 0:
            _b17_n, _b17_lmap = cv2.connectedComponents(_b17_red_px, connectivity=8)
            for _b17_i in range(1, _b17_n):
                _b17_region = (_b17_lmap == _b17_i).astype(np.uint8)
                _b17_filled = ndi.binary_fill_holes(_b17_region).astype(np.uint8)
                result[_b17_filled > 0] = segment_color   # solid red, no holes
        # ── END ADDED ────────────────────────────────────────────────────────────────
        ...
        # Save to the correct classification folder.
        # The reference implementation uses the same indicators/non_indicators
        # folder structure; this version additionally validates the write.
        target_folder = defects_folder if valid_segments >= 1 else non_defects_folder
        target_folder = os.path.abspath(os.path.expanduser(str(target_folder)))
        os.makedirs(target_folder, exist_ok=True)

        safe_filename = os.path.basename(str(filename))
        output_path = os.path.abspath(os.path.join(target_folder, safe_filename))

        if not isinstance(result, np.ndarray) or result.size == 0:
            raise ValueError(f"Cannot save empty result for: {image_path}")

        save_ok = cv2.imwrite(output_path, result)
        if not save_ok or not os.path.isfile(output_path):
            raise IOError(f"OpenCV failed to write output: {output_path}")

        if os.path.getsize(output_path) == 0:
            raise IOError(f"Output file is empty: {output_path}")

        print(f"[BATCH SAVE OK] {output_path}", flush=True)
        ...
        try:
            ...
            pass
        gc.collect()
        _elapsed = _time.perf_counter() - _started_at
        print(
            f"[BATCH DONE] {os.path.basename(str(image_path))} | "
            f"segments={valid_segments} | elapsed={_elapsed:.2f}s",
            flush=True
        )
