...
#
# For FILES THAT THE APP WRITES (e.g. configurations/conf.json), we must
# use the directory that contains the .exe itself — that is always writable
# and is where the user expects application data to live alongside the
# executable.  When running as a plain .py script the behaviour is unchanged.
# ============================================================================
def _get_app_dir() -> Path:
    ...
    * Plain script → directory that contains this .py  (__file__ parent)
    ...
    Use this wherever the application needs to READ or WRITE persistent files
    such as configurations/conf.json.  Never use Path(__file__).resolve().parent
    directly, because __file__ points into the read-only PyInstaller temp
    folder (_MEIPASS) when the app is frozen.
    """
...
class ImageAnalyzerApp:
    ...
    def _batch3d_pick_input_folder(self):
        ...
        # The third candidate uses _get_app_dir() so that, when running as a
        # PyInstaller .exe, the lookup targets the directory that contains the
        # executable (writable, user-visible) rather than the read-only
        # _MEIPASS temp folder that __file__ would resolve to when frozen.
        conf_candidates = [
            os.path.join(folder_path, 'Configurations', 'config.json'),
            os.path.join(folder_path, '..', 'Configurations', 'config.json'),
            str(_get_app_dir() / 'configurations' / 'conf.json'),
        ]
        loaded = False
...
"""
    ...
    def _step1_auto_save(self):
        ...
        # _get_app_dir() returns the .exe directory when frozen by PyInstaller,
        # or the script directory when running as plain .py — both are writable.
        # Path(__file__).resolve().parent must NOT be used here because it
        # resolves to the read-only _MEIPASS temp folder in a frozen .exe.
        try:
            conf_dir = _get_app_dir() / 'configurations'
